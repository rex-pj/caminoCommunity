        ...
        public async Task<bool> UpdateAsync(ProductPicturesModifyRequest request)
        {
            ...
            foreach (var picture in request.Pictures)
            {
                if (!string.IsNullOrEmpty(picture.Base64Data))
                {
                    ...
                    _productPictureRepository.Insert(new ProductPicture()
                    {
                        ProductId = request.ProductId,
                        PictureId = newPicture.Id,
                        PictureTypeId = productPictureTypeId
            ...
            var firstRestPicture = await _productPictureRepository.FindAsync(x => x.ProductId == request.ProductId && x.PictureTypeId != pictureTypeId);
