...
namespace Camino.Application.Validators
{
    ...
    {
        public bool IsValid(UserPhotoUpdateRequest value)
        {
            if (value.FileData == null || value.FileData.Length == 0)
            {
                Errors = GetErrors(new ArgumentNullException(nameof(value.FileData)));
            }

            var image = ImageUtils.FileDataToImage(value.FileData);
            if (image.Width < 100 || image.Height < 100)
            {
                Errors = GetErrors(new PhotoSizeInvalidException(nameof(value.FileName)));
            }
