    ...
    public class UserResolver : BaseResolver, IUserResolver
    {
        ...
        public async Task<UserTokenResult> SigninAsync(SigninModel criterias)
        {
            try
            {
                ...
                var token = await _userManager.GenerateUserTokenAsync(user, ServiceProvidersNameConst.CAMINO_API_AUTH, Camino.Framework.SessionManager.Core.IdentitySettings.AUTHENTICATION_TOKEN_PURPOSE);
                await _userManager.AddLoginAsync(user, new UserLoginInfo(ServiceProvidersNameConst.CAMINO_API_AUTH, token, Camino.Framework.SessionManager.Core.IdentitySettings.AUTHENTICATION_TOKEN_PURPOSE));
        ...
        public async Task<ICommonResult> ForgotPasswordAsync(ForgotPasswordModel criterias)
        {
            try
            {
                ...
                var result = await _userManager.SetAuthenticationTokenAsync(user, ServiceProvidersNameConst.CAMINO_API_AUTH, Camino.Framework.SessionManager.Core.IdentitySettings.RESET_PASSWORD_PURPOSE, resetPasswordToken);
